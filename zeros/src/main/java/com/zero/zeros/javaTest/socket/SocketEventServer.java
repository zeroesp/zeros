...
public class SocketEventServer {
  ...
  private static final int PORT = 17888;
  ...
  private static String generateEvent() {
    ...
    return String.format("user-%s", userNumber) + DELIMITER + event + DELIMITER + new SimpleDateFormat("yyyy/MM/dd HH:mm:ss.SSS").format(new Date());
  }
